...
export async function ingestJob(formData: FormData): Promise<void> {
  const rawDescription = String(formData.get("rawDescription") ?? "").trim();
  // The textarea is `required`, so this only fires if the form is submitted
  // programmatically. Bounce back with a message rather than throwing.
  if (!rawDescription) redirect("/jobs/new?error=empty");
...
export async function analyseJobInternal(jobId: string, opts: { withAi: boolean }) {
  ...
  // The AI result is spread at the top level, not nested: the caller is a
  // generic AiAction button that looks for `mode`/`bridgePrompt` there in order
  // to open the paste-back dialog. Nesting it silently disables bridge mode.
  return { ...result, ok: true, score: match.score };
}
