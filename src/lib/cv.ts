...
/**
 * What an entry should be called on the CV.
 *
 * For a job, the reader wants the job title ("Research & Casework Intern"). For
 * a dissertation or a project, the role is generic and uninformative
 * ("Student", "Undergraduate researcher") while the title carries the substance,
 * so the two categories are led differently.
 */
export function cvHeadlineFor(atom: { title: string; role: string; category: string }): string {
  if (["ACADEMIC", "PROJECT", "CERTIFICATION"].includes(atom.category)) {
    return atom.title || atom.role;
  }
  return atom.role || atom.title;
}
