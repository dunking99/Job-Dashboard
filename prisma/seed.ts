...
import { yearsOfExperience } from "../src/lib/candidate";
...
We reserve the right to close this vacancy early if we receive sufficient applications.`,
  ...
  // Score the demo jobs the same way ingestion would, so the dashboard shows a
  // working system rather than a row of dashes on first run.
  await scoreAllJobs();
...
/**
 * Run the deterministic analysis pass over every job. This is the same
 * computation `analyseJobInternal` performs on ingest — duplicated here rather
 * than imported because the seed runs outside Next and must not pull in the
 * server-action module graph.
 */
async function scoreAllJobs() {
  const [jobs, atoms, skills, profile] = await Promise.all([
  ...
  const targetSectors: string[] = JSON.parse(profile?.targetSectors ?? "[]");
  const years = yearsOfExperience(atoms);
  ...
  for (const job of jobs) {
    const match = computeMatch(
      ...
      atoms as never,
      skills.map((s) => s.name),
      { targetRoles, targetSectors, yearsExperience: years }
    );
    ...
    await prisma.job.update({
      ...
      data: {
        ...
        matchBreakdown: JSON.stringify(match.components),
        gaps: JSON.stringify(match.gaps),
        extractedSkills: JSON.stringify(match.extracted),
        yearsRequired: match.yearsRequired,
        redFlags: JSON.stringify(detectRedFlags(job.rawDescription)),
        analysedAt: new Date(),
      },
    });
  }
